...
import { buildRequirementRegistry } from "./requirements.js";
...
import { buildDictionaryExpansion, buildSearchIndex, deserializeSearchIndex, flattenWorkspace } from "../search/index.js";
const READ_MODEL_MEMO_LIMIT = 8;
function createEmptyMemoStats() {
    return { hits: 0, misses: 0, size: 0 };
}
const readModelMemoState = {
...
export function clearReadModelMemo(root) {
    readModelMemo.clear(root === undefined ? undefined : resolve(root));
}
export function getReadModelMemoStats() {
    return { ...readModelMemoState.stats };
}
export function resetReadModelMemoStats() {
...
async function buildReadModel(root, sections) {
...
async function regenerateStaleReadableCacheSections(root, cacheMode, sections) {
    const uniqueSections = [...new Set(sections)];
    const staleReadableSections = await Promise.all(uniqueSections.map(async (section) => {
...
async function loadSourceReadModel(root, sections, diagnostics) {
...
function createReadModel(root, sections, state) {
    return {
        root,
        sections,
        stats: state.stats,
        ...
        buildGraph: (graphType) => {
            ...
            const diagnostics = state.workspace === undefined
...
async function buildReadModelCacheKey(root, cacheMode, sections) {
    ...
    const sourceIntegrityHash = cacheMode !== "bypass" && artifactIntegritySummaries.every((summary) => summary.sha256 !== undefined)
...
async function artifactIntegrityInputs(root, sections) {
    const paths = new Set();
...
async function requirementShardHashes(root, guard) {
    if (guard === undefined) {
...
async function artifactHashSummary(root, storePath, guard) {
    if (guard === undefined) {
...
async function hashManifestOrMissing(root) {
    try {
        const target = await resolveRealStorePathWithGuard(root, normalizeStorePath(cacheOutputStorePaths.manifest), await createRealPathGuard(root));
