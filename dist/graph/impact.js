...
export function impactRequirement(input, graph) {
    if (!graph.ok) {
    ...
    const traversalEdgeKeys = new Set();
    const queue = [{ key: rootKey, id: requirementId, depth: 0, via: [requirementId] }];
...
function impactTransitions(currentKey, edges) {
    ...
    return transitions.sort((left, right) => relationRank(left.edge.relationType) - relationRank(right.edge.relationType) ||
        left.nextKey.localeCompare(right.nextKey) ||
        compareGraphEdges(left.edge, right.edge));
}
function buildImpactContext(nodes, edges, rootKey, impactedKeys, traversalEdgeKeys, options) {
    const nodeByKey = new Map(nodes.map((node) => [node.key, node]));
    const edgeByKey = new Map(edges.map((edge) => [edge.key, edge]));
    const includedNodeKeys = new Set([rootKey, ...impactedKeys]);
    const includedEdgeKeys = new Set(traversalEdgeKeys);
