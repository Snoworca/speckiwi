...
import { isInsideDirectory } from "../io/path.js";
...
import { exportPathForDocument, renderDiagnosticsSummary, renderDocumentMarkdown, renderExportIndex } from "./templates.js";
...
function selectTargetDocuments(documents, manifestEntries, types, documentIds) {
...
function entryMatches(entry, types, documentIds) {
...
function documentMetadata(value) {
    const metadata = {};
    for (const key of ["title", "status", "scope"]) {
        if (typeof value[key] === "string") {
            metadata[key] = value[key];
        }
    }
    if (Array.isArray(value.tags)) {
        const tags = value.tags.filter((tag) => typeof tag === "string").sort();
        if (tags.length > 0) {
            metadata.tags = tags;
        }
    }
...
function normalizeExportTypes(value) {
...
async function assertNoSymlinkTraversal(outputRoot, absolutePath) {
    try {
...
function isExportableDocumentType(value) {
    return value === "overview" || value === "srs" || value === "prd" || value === "technical" || value === "adr";
}
function compareTargetDocuments(left, right) {
