...
export function buildProgram() {
    ...
    init.action(() => executeCliCommand(init, async (context) => {
        const input = {
            root: context.root,
    ...
    validate.action(() => executeCliCommand(validate, async (context) => validateWorkspace(context), { exitCode: validationExitCode }));
    const doctorCommand = addCommonOptions(program.command("doctor").description("check local runtime and workspace health"));
    doctorCommand.action(() => executeCliCommand(doctorCommand, async (context) => doctor(context), {
        resolveWorkspace: false,
