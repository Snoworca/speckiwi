...
| Canonical Location | `docs/rule/SRS-MD-Rules-v3.0.0.md` |
...
| Path | Responsibility |
|---|---|
| `docs/rule/SRS-MD-Rules-v3.0.0.md` | SRS-MD authoring, parsing, validation, agent workflow rules |
...
§7 Completed Work Log heading은 index에 반드시 존재해야 하지만, 그 inline data row는 선택적이다. row는 별도 history 파일(`docs/spec/91.completed-work-log.md`)에 둘 수 있으며 parser가 양쪽을 병합한다(§7.4 참고).
...
11. Completed Work Log는 `00.index.md` §7 inline, 별도 history 파일 `docs/spec/91.completed-work-log.md`, 또는 둘 다에 존재할 수 있다. Parser는 두 source를 모두 읽어 `index.completedWork`로 병합한다(dual-read).
12. history 파일은 `.srs.md`가 아닌 일반 `.md`이며 scope/step 파일로 발견되지 않는다. heading 위에 read-only summary banner(완료 판정의 source of truth가 아님을 명시)를 두고, §7 와 동일한 table grammar(legacy five-column 및 trailing Report Paths six-column)를 따른다.
13. 병합은 append-concatenation(index row 다음 history row)이며 정렬·중복 제거는 query 계층에서 수행한다. `Date`/`Target`/`Requirement IDs`/`Summary`가 동일하여 두 source에 중복된 row는 dedup하고 `SRS-W025` warning으로 보고한다.
14. 신규 `add_completed_work` row는 history 파일에만 기록한다(부재 시 banner와 함께 bootstrap). inline index row는 하위호환을 위해 계속 읽되 새로 쓰지 않는다. inline row를 history 파일로 옮기는 마이그레이션은 opt-in이며 기본은 dry-run이다.
15. history 파일은 append-only summary로, per-row·status-independent이며 bulk-finalize 금지 정책을 약화하지 않는다.
16. §7 Completed Work Log 진단(`SRS-W011`~`SRS-W015`, `SRS-W024`)은 각 entry의 originating 파일(history 또는 index)을 location으로 보고한다.
...
| `related_to` | 일반 관련 관계. |
| `checked_compatible` | (compatibility cache) 두 요구사항이 특정 semanticSha 시점에 모순 없이 양립함을 기록한 advisory 캐시 행 — §23.5 참조. |
...
`checked_compatible` 는 두 요구사항이 특정 내용 시점(semanticSha)에 모순 없이 양립함을 기록하는 **advisory 캐시 관계**다. 일반 관계와 달리 다음 규칙을 따른다.

1. **Notes 토큰 문법**: Trace Links 행의 `Notes` 셀은 `key: value` 항목을 `; ` (semicolon-space) 로 구분하며, 키는 소문자·하이픈만 허용한다. 인식 키: `fpv` (formula/포맷 버전, 예 `fpv1`), `self` (본 요구사항 semanticSha), `peer` (상대 요구사항 semanticSha), `checked-at` (검증 시각/마커). 값 charset 은 제한적이다(영숫자·하이픈·콜론·점 등; 줄임표나 임의 문자는 비허용). `table-cell.ts` 의 셀 안전성(`|`/CR/LF 거부)만으로는 부족하며 전용 토크나이저(`parseCompatibilityNotes`)가 charset 을 강제한다.
2. **semanticSha 정규화 계약 (fpv1)**: `self`/`peer` 핀은 요구사항의 정규화된 내용 해시다. 정규화 = CRLF→LF, 라인별 trailing whitespace 제거, whitespace run 1칸 축소, 외곽 trim. 해시 입력 = 정규화된 requirement 문 + AC 텍스트(checked 상태 제외) + status + stability + metadata(DENY={Status, Stability}, Scope 포함); Trace Links / Verification Evidence / Change Notes 는 제외한다.
3. **validator treatment**: `checked_compatible` 행은 **referential-existence 만 검증**한다(참조 ID 존재). **liveness(핀 stale 여부·endpoint status/stability)는 강제하지 않는다** — stale/missing/conflict 는 advisory warning(SRS-W041/W042/W043)으로만 보고하며 release gate 를 막지 않는다. 행 생성·갱신·폐기는 전용 mutation 도구로만 수행한다.
...
### 30.1 Discarded 마커 (v1.1.0 신설)

`update_status` mutation 이 `Status=discarded` 로 전이될 때, 해당 Requirement Block 의 heading 에 다음 마커가 **자동 적용** 된다. 작성자가 별도 `Edit` 으로 마커를 부착할 책임이 없으며, mutation 1회 호출이 line-patch 1회로 완결된다 (Markdown 원본 원칙 §2.1 보존).
...
단일 `supersedes` trace_link 가 존재할 때:

```
### ~~REQ-ID — Title~~ [DISCARDED → see REQ-Y]
```

다중 `supersedes` trace_link 가 존재할 때 (`+N` 표기):

```
### ~~REQ-ID — Title~~ [DISCARDED → see REQ-Y +2]
```

여기서 `REQ-Y` 는 **Trace Links 표의 row 등장 순서 상 최상단** 1건 (= FIRST) 이며, `+N` 은 잔여 supersedes row 의 개수이다. FIRST 결정 규칙:

- 본 Requirement Block 의 `Trace Links` 표를 위에서 아래로 스캔.
- `Type=Requirement`, `Relation=supersedes`, `Reference=<본 REQ-ID>` 와 매치하는 모든 row 를 등장 순서로 정렬.
- 첫 row 의 `Reference` 가 가리키는 REQ-ID 가 `see` 슬롯 대상.
- 잔여 row 개수 = `N`.

부활 (다른 status 로 재전이) 시 mutation 은 strikethrough (`~~`) 와 `[DISCARDED ...]` 마커를 모두 자동 제거하여 원본 형태 `### REQ-ID — Title` 로 복원한다.
...
단일 `conflicts_with` trace_link 가 존재할 때:
...
`see` 슬롯 / `+N` 결정 규칙 — §30.1 과 동일한 4-step:

- 본 Requirement Block 의 `Trace Links` 표를 위에서 아래로 스캔.
- `Type=Requirement`, `Relation=conflicts_with`, `Reference=<본 REQ-ID>` 와 매치하는 모든 row 를 등장 순서로 정렬.
- 첫 row 의 `Reference` 가 가리키는 REQ-ID 가 `see` 슬롯 대상.
- 잔여 row 개수 = `N`.

부활 (다른 status 로 재전이) 시 mutation 은 `[DRAFT ...]` 마커를 자동 제거하여 원본 형태 `### REQ-ID — Title` 로 복원한다.

### 30.3 Tool Compliance (v1.1.0 신설)

mutation, parser, validator, renderer 가 §30.1 / §30.2 표기를 동시에 인식·생성해야 한다. 어느 한 component 가 v1.0.0 모드만 지원하면 binding inconsistency 로 mutation 이 거부된다 (구현 책임은 speckiwi 측이 정의).

`update_status` 호출은 다음을 단일 line-patch 트랜잭션으로 적용한다:

1. heading 라인의 마커 토글.
2. metadata 표의 `Status` row 갱신.
...
기존 SRS 가 임의로 `[OBSOLETE]`, `[deprecated]`, 빈 strikethrough 등 §30.1 / §30.2 규약과 다른 표기를 가지는 경우, 본 v1.1.0 parser 는 **unknown marker warning** 만 발행하고 본문 정규 매칭을 시도하지 않는다 (lenient runtime stage).

비표준 마커를 표준으로 자동 정리하는 책임은 별도 도구 (예: `speckiwi migrate-markers`) 에 위임된다. 사용자가 의도적으로 호출하는 migration CLI 는 strict stage 로서 비표준 발견 시 default fail + JSON 리포트를 출력한다 (구체 명세는 speckiwi 측 migration 도구 정의). 두 stage 의 정책 차이는 의도적이다 — parser 런타임은 lenient (warning), migration CLI 는 strict (fail).

heading title 영역 (`### REQ-ID — Title`) 의 title 부분에 등장하는 `[...]` 패턴 중 다음 whitelist 외 표기는 unknown marker warning 후보가 된다 (sub-parser 책임):

- whitelist: `[<REQ-ID>]` 형식 (예: `[FR-AUTH-002]`) — dependency 인용 또는 cross-reference.
- blacklist: 위 외 모든 `[...]` 토큰 (예: `[TBD]`, `[NOTE]`, `[OBSOLETE]`) — warning.

엄격 정책 디폴트는 PR-A merge 후 운영 데이터로 확장 검토.

### 30.5 호환성 (v1.1.0 신설)

본 v1.1.0 규약의 활성화 여부는 해당 SRS 패키지의 `docs/spec/90.appendix.md` 메타 표의 **`Rules` 행** 링크 파일명에 포함된 버전 토큰으로 판정한다. frontmatter 는 사용하지 않는다 (Markdown 원본 원칙 §2.1 보존).
...
- 키 표기: `| Rules | ... |` (canonical) 또는 `| rules | ... |` (case-insensitive normalize 후 동일하게 인정).
- 좌우 공백 padding: markdown 표 parser 단계에서 cell trim 된 후 정규식 입력으로 전달된다.
- 링크 본문: 상대 경로 (`../rule/...`) 와 저장소-루트 절대 경로 (`./docs/rule/...`) 모두 인정.

링크 파일명에 추출된 version 토큰이 `1.1.0` 이상이면 본 §30.1 / §30.2 마커 자동 적용이 활성화되며, parser / validator / renderer / mutation 이 §30.x 규약을 강제한다. v1.0.0 mode 인 SRS 에서 §30.1 / §30.2 표기가 등장하면 parser warning 후 mutation 차단.
...
이 문서는 SRS-MD Authoring Rules v3.0.0의 기준 문서다. 저장소별 상황에 맞는 세부 enum, scope prefix, target naming은 `docs/spec/00.index.md`와 `docs/spec/90.appendix.md`에서 확장할 수 있다. 단, Requirement Block의 heading 형식, metadata table, status 의미, Acceptance Criteria, Verification Evidence, Trace Links 규칙은 호환성을 위해 유지해야 한다.

v1.1.0 신설 항목은 §30.1 ~ §30.5 이다 (Discarded / Draft 마커 자동 적용, Tool Compliance, 비표준 마커, 호환성 판정 규칙). v1.0.0 의 §1 ~ §40 본문은 변경 없이 보존되며 §30 안에 §30.1 ~ §30.5 가 보강되었다. v1.0.0 의 §31 이후 절번호 (§31 ~ §41) 는 그대로 유지된다.
