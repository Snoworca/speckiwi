...
// @req FR-FLOW-027 AC-4
//
// §0.2 declares that the skill mutates neither the filesystem nor git, while §2.6.1 instructs it to
// run `git worktree add`. The requirement body already voided the read-only claim — "This extends
// kiwi-pipeline beyond its prior read-only nature ... to a defined git worktree mutation" — but the
// skill text never followed, and no assertion covered §0.2, so a verified requirement changed
// underneath it without anything turning red. An agent reading §0 first can refuse §2.6 on the
// strength of a rule the requirement retired.
//
// Four renderings, not three: the `.agents` mirror ships too. It is regenerated by
// `speckiwi skills mirror --write` from the codex source and must never be hand-edited.

const PIPELINE_RENDERINGS: ReadonlyArray<readonly [string, string]> = [
  ["claude", path.join(REPO_ROOT, "skills", "claude", "kiwi-pipeline", "SKILL.md")],
  ["codex", path.join(REPO_ROOT, "skills", "codex", "kiwi-pipeline", "SKILL.md")],
  ["etc", path.join(REPO_ROOT, "skills", "etc", "kiwi-pipeline", "SKILL.md")],
  ["agents-mirror", path.join(REPO_ROOT, ".agents", "skills", "kiwi-pipeline", "SKILL.md")]
];

/** The §0.2 table row, which is where the self-mutation ban is stated in every rendering. */
function selfMutationRow(text: string): string {
  const row = /^\|\s*§0\.2\s*\|.*$/m.exec(text);
  return row === null ? "" : row[0];
}

describe("FR-FLOW-027 AC-4 — §0.2 names the §2.6 worktree creation as its exception", () => {
  for (const [rendering, file] of PIPELINE_RENDERINGS) {
    describe(`rendering: ${rendering}`, () => {
      it("states a §0.2 self-mutation rule at all", () => {
        expect(selfMutationRow(readFileSync(file, "utf8")), `${rendering}: §0.2 row not found`).not.toBe("");
      });

      it("qualifies the ban with an explicit exception rather than banning git mutation outright", () => {
        const row = selfMutationRow(readFileSync(file, "utf8"));
        expect(/예외|exception/i.test(row), `${rendering}: §0.2 must mark an exception, not state an absolute ban`).toBe(
          true
        );
      });

      it("points the exception at the worktree creation it actually performs", () => {
        const row = selfMutationRow(readFileSync(file, "utf8"));
        expect(
          /§2\.6|--wt|FR-FLOW-027/.test(row),
          `${rendering}: §0.2's exception must name §2.6 / --wt / FR-FLOW-027 so the reader can find what it permits`
        ).toBe(true);
      });
    });
  }
