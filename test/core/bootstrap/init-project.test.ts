...
describe("project init bootstrap", () => {
  it("creates docs/spec, docs/rule, canonical rules, index, appendix, and both agent snippets", async () => {
    ...
    const appendix = await readFile(path.join(rootPath, "docs", "spec", "90.appendix.md"), "utf8");
    expect(appendix).toContain("Report Paths");
    expect(appendix).toContain("--report <path>");
    ...
    expect(agents).toContain("# SpecKiwi SRS 워크플로 v1.4");
    ...
    expect(agents).toContain("Agents MUST follow TDD for behavior changes");
    ...
    expect(await readFile(path.join(rootPath, "CLAUDE.md"), "utf8")).toContain("# SpecKiwi SRS 워크플로 v1.4");
    ...
    expect(await readFile(path.join(rootPath, "CLAUDE.md"), "utf8")).toContain("Next target authoring workflow:");
  });
  ...
  it("is idempotent and skips duplicate agent snippets", async () => {
    ...
    expect(agents.match(/# SpecKiwi SRS 워크플로 v1\.4/g)).toHaveLength(1);
    expect(agents.match(/<!-- \/SpecKiwi SRS 워크플로 -->/g)).toHaveLength(1);
    expect(claude.match(/# SpecKiwi SRS 워크플로 v1\.4/g)).toHaveLength(1);
  ...
  it("replaces stale versioned and legacy agent instruction blocks", async () => {
    ...
    expect(claude).toContain("# SpecKiwi SRS 워크플로 v1.4");
