...
describe("project init bootstrap", () => {
  it("creates docs/spec, docs/rule, canonical rules, index, appendix, and both agent snippets", async () => {
    ...
    expect(await readFile(path.join(rootPath, "CLAUDE.md"), "utf8")).toContain("docs/rule/SRS-MD-Rules-v1.0.0.md");
  });
  ...
  it("is idempotent and skips duplicate agent snippets", async () => {
    ...
    expect(agents.match(/SRS-MD-Rules-v1\.0\.0\.md/g)).toHaveLength(1);
    expect(claude.match(/SRS-MD-Rules-v1\.0\.0\.md/g)).toHaveLength(1);
  });
