...
async function writeSource(root: string, entrypoint: "SKILL.md" | "skill.md", body: string): Promise<void> {
  await mkdir(path.join(root, "kiwi-demo"), { recursive: true });
  await writeFile(path.join(root, "kiwi-demo", entrypoint), body, "utf8");
...
async function setup(): Promise<Fixtures> {
  ...
  await writeSource(claudeSource, "skill.md", "bundled demo skill\n");
  await writeSource(codexSource, "SKILL.md", "bundled demo skill\n");
