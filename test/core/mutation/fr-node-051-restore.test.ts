...
// FR-NODE-062 — restore core un-discards a requirement with required reason.
...
// against the future `restore` behaviour described in FR-NODE-062. These cases fail
 ...
 * blocked by the FR-NODE-035 protected-state guard.
 */
 ...
 * Status=verified) and then discard it via the FR-NODE-035 override path. This is the
...
describe("FR-NODE-062 restore core un-discards a requirement with required reason", () => {
  // AC-1: restore on a discarded requirement sets its Status to the requested active
  // status, defaulting to planned.
  it("FR-NODE-062 AC-1: defaults restored Status to planned, and honours an explicit active status", async () => {
  ...
  // requirement heading.
  it("FR-NODE-062 AC-2: removes the heading strikethrough and DISCARDED marker", async () => {
  ...
  // patch.
  it("FR-NODE-062 AC-3: appends exactly one Change Notes row carrying the supplied reason", async () => {
    const rootPath = await copyFixtureWorkspace("mutation-target");
    await makeDiscarded(rootPath);
  ...
  it("FR-NODE-062 AC-4: without a reason returns ok false and writes no file", async () => {
    const rootPath = await copyFixtureWorkspace("mutation-target");
    await makeDiscarded(rootPath);
    const before = await readFile(await specPath(rootPath), "utf8");
  ...
  it("FR-NODE-062 AC-5: restoring a previously-verified requirement emits a stale AC/evidence warning", async () => {
    const rootPath = await copyFixtureWorkspace("mutation-target");
