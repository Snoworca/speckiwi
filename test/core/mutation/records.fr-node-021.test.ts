...
// FR-NODE-021 — compareReqId raw-byte ordering and computeBlastRadius closure utility.
//
...
describe("FR-NODE-021 compareReqId raw-byte ordering and computeBlastRadius closure utility", () => {
  ...
  it("FR-NODE-021 AC-1: compareReqId orders REQ-IDs by raw byte comparison", () => {
    ...
    expect(compareReqId("FR-NODE-021", "FR-NODE-021")).toBe(0);
    ...
    const forward = compareReqId("FR-CLI-010", "FR-NODE-021");
    const reverse = compareReqId("FR-NODE-021", "FR-CLI-010");
  ...
  it("FR-NODE-021 AC-2: computeBlastRadius returns the depends_on closure bounded to 2 hops", () => {
    const records = [
  ...
  it("FR-NODE-021 AC-3: computeBlastRadius cuts traversal at frozen/stable edges", () => {
    const records = [
  ...
  it("FR-NODE-021 AC-4: computeBlastRadius is pure with no side effects", () => {
    const records = [
