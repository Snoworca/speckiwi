...
- **자율 결정 원칙 예외 2건** — frozen REQ 변경 / push 충돌. 둘 다 비가역·고위험이므로 standalone 모드에서는 User clarification gate, child 모드에서는 NEEDS_USER bubble-up.
- **speckiwi 연동 필수** — SpecKiwi REQ/Task/SRS 연동이 필요한 normal workflow 에서는 MCP 부재 또는 mutation 실패 시 HALT 한다. 순수 Git/GitHub 만 수행하려면 사용자가 `--no-speckiwi` 를 명시해야 한다.

### 11.1 입력 컨텍스트 자동 감지 (Step 3 확장)

Step 3.1 의 후보 수집과 **순차 단일로** speckiwi 컨텍스트를 자동 추출:

| 우선 | 출처 | 추출 방법 |
|---|---|---|
| 1 | **호출자 prompt 인자** | kiwi-pm / kiwi-coder 가 본 스킬을 delegated worker로 spawn 할 때 prompt 에 직접 주입한 `task_id` / `req_ids` / `run_id` (§11.6 인계 프로토콜 참조) |
...
`speckiwi mcp` 부재 시: SpecKiwi 출처 4 와 모든 SRS mutation/trace/evidence 갱신을 진행하지 않는다. `--no-speckiwi` 로 순수 Git/GitHub commit 흐름을 명시한 경우에만 REQ trailer 없이 진행할 수 있고, 그렇지 않으면 MCP 복구 안내 후 HALT 한다. CLI 는 진단/복구 안내에만 사용한다.

### 11.2 REQ-ID / Task-ID 매칭 평가 (Step 3.3 확장)

Step 3.3 의 local evaluator 평가자에 평가 축 2종 추가:

| 축 | A+ 조건 |
|---|---|
| req_match | 후보 REQ 의 acceptance criteria 가 diff 로 충족됨 (`check_acceptance_criteria` MCP 결과 또는 local evaluator 자체 평가) |
| task_match | 후보 plan task 의 `files[]` 와 diff 변경 파일이 일치 + `dod` 가 diff 로 충족 |

**판정**:
- 두 축 모두 A+ + 동일 evaluator 3회 연속 no actionable improvement → `REQ: FR-XXX-001` + `Task: T-PHnnn-mm` trailer 부착
- REQ 만 A+ + 동일 evaluator 3회 연속 no actionable improvement → `REQ:` trailer 만
...
| `frozen` | **User clarification gate 3옵션** (자율 결정 원칙 예외 — frozen 변경은 비가역·범위 외 위험): (1) `STABILITY-OVERRIDE: <reason>` trailer 추가 후 진행 / (2) commit 중단 / (3) REQ 의 stability 갱신 후 재시도 (`$kiwi-srs-feasibility` 호출 권고) | `NEEDS_USER` 상태로 즉시 bubble-up. `decision_options` 에 standalone 3옵션을 직렬화 (§11.6 NEEDS_USER 스키마 참조). kiwi-pm 의 §0.G2 lifecycle gate 와 정합 |
...
| MCP 부재로 stability 미확인 | HALT (`--no-speckiwi` 명시 시 trailer skip) | NEEDS_USER/FAILED 로 bubble-up |
...
- sidecar 파일 직접 manual file edit (kiwi-planner 의 황금률은 `docs/spec/*.srs.md` mutation 후 동일 SRS 파일 manual SRS file edit 만 차단하며, sidecar JSON 의 mcp_call_log 외 필드 수동 편집은 허용한다는 정책에 정합. 정확 절번호는 kiwi-planner SKILL.md 본문 확인 — 변경 시 본 절 보정)

#### 11.5.4 MCP 부재 / mutation 실패

- speckiwi MCP 부재 → §11.5 전체 HALT 또는 `--no-speckiwi` 명시 시 전체 skip + WARN ("speckiwi 연동 없음, SRS mutation/trace/evidence 미수행")
- 단일 mutation 실패 → HALT 하고 실패한 MCP 호출, 입력, 응답을 보고한다. 다음 mutation 으로 진행하지 않는다.
- 모든 mutation 실패 후 정상 TASK_DONE 으로 보고하지 않는다.

### 11.6 `$kiwi-coder` / `$kiwi-pm` 와의 인계 프로토콜

본 스킬이 위임 worker로 호출되었고 호출자 prompt 에 다음 형태의 컨텍스트가 명시되어 있을 때 **child 모드** 로 전환:
...
(kiwi-pm 은 위임 worker prompt 에 직접 위 컨텍스트를 인라인으로 주입한다. 환경변수 인계는 가정하지 않는다.)

#### 11.6.1 child 모드 동작 변경

- User clarification gate 비활성 — kiwi-pm 의 3상태 프로토콜(TASK_DONE / NEEDS_USER / FAILED) 로 bubble-up
...
|---|---|
| frozen REQ → User clarification gate 3옵션 | NEEDS_USER (reason: stability_frozen, severity: business-decision, decision_options: 동일 3옵션 직렬화) |
| push 충돌 → User clarification gate 3옵션 (rebase/merge/중단) | NEEDS_USER (reason: push_conflict_non_fast_forward, severity: business-decision) |
| rebase/merge 파일 충돌 | NEEDS_USER (reason: push_conflict_rebase / push_conflict_merge) |
| issue 후보 매칭 모호 (예: local evaluator 평가가 모든 후보 A 이하) | NEEDS_USER (reason: issue_candidate_ambiguous, severity: clarification — `--auto` 시 자동 trailer skip 처리) |
| gh CLI 인증 실패 | FAILED (error: gh_cli_not_installed_required, partial_state.commit/push 채움) |
| speckiwi MCP 호출 모두 실패 | FAILED 또는 NEEDS_USER — `--no-speckiwi` 명시 없는 normal workflow 에서 TASK_DONE 금지 |
...
| `local-LLM max profile` | kiwi 시리즈 일관성 위해 추가 (현재는 local evaluator 평가자가 기본이라 no-op, 미래 local evaluator 평가자 도입 시 활성) |
