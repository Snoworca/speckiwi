...
- verified 후보
- 남은 작업 제안
- 12. MCP diagnostics (CRITICAL 차단 해제)
...
|---|---|---|---|
| **Missing-Detector** | local evaluator | local-LLM max-profile | `inventory.json` 의 모든 항목이 최소 1개 요구사항에 매핑되는지 전수 점검 |
| **Correctness-Auditor** | local evaluator | local-LLM max-profile | SRS 항목의 statement·AC가 실제 코드 동작과 일치하는지 검증 |
| **Hallucination-Hunter** | local evaluator | local evaluator | **단정 서술 + 코드 증거 전무** 항목 발견 (거짓 사실 진술) |
| **Scope-Creep-Reviewer** | local evaluator | local evaluator | **추측 서술 + 코드 무관** 항목 발견 (향후 요구·임의 작성) |

**비용 가드레일**: 1st iter는 모두 local evaluator. 2nd+ iter에서 동일 axis가 동일 finding을 다시 보고하면 local-LLM max-profile 로 escalate. 검증자 출력은 파일(`eval_iter{N}.json`)로만 저장하고 메인 컨텍스트에는 summary count + top 3 CRITICAL/HIGH 만 로드.
...
iter당 finding 이 0건인 scope 의 개선 agent 는 spawn 하지 않음. finding 보유 scope 만 순차 단일 spawn.
...
2. type 별 처리 (모두 MCP 정식 도구만 사용, manual file edit through the host file edit mechanism 금지):
...
4. 단일 evaluator 가 개선사항 없음(no new actionable finding)을 3회 연속 반환
...
단일 evaluator 가 no-improvement 를 3회 연속 반환하면 Phase 7 로 진행한다. 단순 finding_hash 부재만으로 조기 종료하지 않는다.

### 10.4 max-eval-iter 기본값

`--max-eval-iter` 는 안전 상한이며 기본 **9**. 단, 정상 종료 조건은 항상 3회 연속 no-improvement 이다. 상한에 먼저 도달하면 사용자에게 반복 한계와 잔여 finding 을 보고하고 HALT 한다.
...
- manual file edit through the host file edit mechanism 가 필요한 경우 → 사용자 에스컬레이션 (자동 manual SRS file edit 금지)
...
speckiwi MCP 도구가 정상 경로다. CLI 는 사용자가 MCP 연결을 복구할 때 참고할 진단 예시일 뿐이며, etc 스킬은 CLI 로 SRS mutation/status/trace/evidence 를 수행하지 않는다.

| 작업 | MCP | CLI diagnostic note |
|---|---|---|
| 초기화 | `init_project` | MCP unavailable => halt and remediate |
| Active Target 조회 (단일) | `get_active_target` | diagnostic read only |
| Target 활성화 | `set_active_target` | MCP required |
| 요구사항 추가 | `add_requirement` | MCP required |
| Status 변경 | `update_status` | MCP required |
| Trace 추가 | `add_trace_link` | MCP required |
| Verification 추가 | `add_verification_evidence` | MCP required |
| 검증 | `validate_spec` | diagnostic read only |
| 요약 | `summarize_target` | diagnostic read only |
| 목록 | `list_requirements` | diagnostic read only |
| 요구사항 조회 | `get_requirement` | diagnostic read only |

CLI stdout 은 MCP 복구 진단에만 참고한다. SRS 변경이 필요한 단계에서 MCP 가 없으면 HALT 한다.
