---
name: kiwi-srs-sync
description: "OpenCode/Hermes local-LLM variant for synchronizing implementation results back to SpecKiwi SRS. Requires speckiwi mcp for all normal mutations. Defaults to --max with one evaluator/worker and three consecutive clean evaluations before proceeding. Triggers: kiwi srs sync, implementation-to-SRS sync."
---
...
| §0.9 | **speckiwi MCP 우선 + CLI diagnostics/remediation**. MCP 부재 시 HALT. `speckiwi` CLI는 진단/복구 안내에만 사용 |
...
| IF | THEN |
|---|---|
| 평가자 PASS (3회 연속 no actionable improvement) + `--auto-apply` 없음 | dry-run 산출물 생성 + User clarification gate 4옵션 (apply-all / apply-selected / dry-run-only / abandon) |
| 평가자 PASS (3회 연속 no actionable improvement) + `--auto-apply` 또는 `--yes-all` | 즉시 MCP mutation 진행 + 사후 보고 |
...
| IF | THEN |
|---|---|
| git diff 에 cwd 외부 path 진입 | 즉시 중단 + User clarification gate 3옵션 (cwd 한정 / 외부 포함 진행 / 작업장 이동) |
...
| 분류 | 정의 | 매핑 액션 |
|---|---|---|
| **conflict** | 변경 코드가 기존 REQ 의 AC 또는 dod 와 정면 충돌 (예: AC 가 "X 거부" 인데 코드가 "X 허용") | User clarification gate: (1) REQ AC 갱신 / (2) 코드 rollback / (3) deprecated 처리 |
...
| **new-scope** | 기존 target 의 어느 scope 에도 안 들어가는 신규 영역 | User clarification gate 3옵션: (1) 신규 scope 생성 — **본 스킬 범위 밖, `$kiwi-srs` 위임** (사용자 확정 후 본 스킬 종료) / (2) 기존 scope 확장 — `add_requirement` 호출 시 기존 scope 명 사용 (본 스킬 내 처리) / (3) 다른 target 으로 이동 — **본 스킬 범위 밖, 사용자가 `set_active_target` 수동 실행 후 본 스킬 재실행** |
...
| 모드 | 사전조사 (local evaluator) | 시니어 분석가 (local-LLM max-profile) | 평가자 (local-LLM max-profile + local evaluator) | 비용 배수 |
|---|---|---|---|---|
| default `--max` | × 1 | × 1 | single evaluator, 3 clean evaluations | bounded single-worker |
...
- **분석 디렉토리**: `docs/analysis/kiwi-srs-sync-{run-id}/`
  ...
  - `intent.json` / `srs_mapping.json` / `impact.json` — single local evaluator 사전조사 결과
  - `senior_analysis.json` — local-LLM max-profile 분석가 통합 결과 (REQ 매핑 + AC 갱신 제안 + 신규 REQ 초안)
...
Phase 2 : single local evaluator 사전조사 순차 단일 (intent / srs-existing-mapping / impact)
Phase 3 : local-LLM max-profile 시니어 분석가 (4방향 분류 + REQ 매핑 + AC 갱신 제안 + 신규 REQ 초안)
Phase 4 : single delegated evaluator, 3 consecutive clean evaluations (격리 입력)
Phase 4.5: 개선 루프 (심각도 카운터)
Phase 5 : dry-run 산출물 생성 (proposed-mutations.md/.json)
Phase 6 : 사용자 게이트 (User clarification gate 4옵션) — `--auto-apply` 시 skip
...
3. MCP 없음 → User clarification gate

**TARGET slug 규약**: 사용자 입력값을 그대로 사용 (정규화 없음). 즉 자연어 "타겟 1.0.0" → `TARGET=1.0.0`, "타겟 v1.0.0" → `TARGET=v1.0.0`. **`v` prefix 자동 부여·제거 금지** — speckiwi target 이름이 실제로 `v` 를 포함하는지는 `list_requirements` / `get_active_target` 응답으로만 결정. 사용자가 prefix 없이 입력했으나 활성 target 이 `v1.0.0` 이면 User clarification gate 으로 명시 확인 후 진행.
...
## 5. Phase 2 — single local evaluator 사전조사 (순차 단일, 격리)
...
## 6. Phase 3 — local-LLM max-profile 시니어 분석가 (4방향 분류)
...
3. mapping.candidate_reqs 가 medium 매칭:
   → User clarification gate (모호성, 사용자 결정)
```
...
## 7. Phase 4 — 평가자 (local-LLM max-profile + local evaluator)
...
- CRITICAL/HIGH/MEDIUM finding 은 개선사항으로 간주하고 §7.5 개선 후 clean counter 를 0으로 재설정
