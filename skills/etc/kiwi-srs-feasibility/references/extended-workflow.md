...
- 5.5.6 비용 가드
- 5.5.7 산출물
- 6. Phase 3 — Target-wide synthesis (local-LLM max-profile 시니어)
...
`--enable-research` 명시 시에만 활성. `kiwi-srs-research` 스킬을 **delegated worker 모드** 로 호출하여 모호한 블로커/조건을 가진 REQ 에 대한 연구 결과를 per-REQ judgement 에 병합.
...
사용 가능한 위임 worker 위임 도구로 default 연구 에이전트 1개를 실행한다. 메시지에는 아래 토큰과 본문을 모두 포함한다.

```text
kiwi-srs-research delegated worker for FR-TODO-005 --mode=delegated worker

kiwi-srs-research 스킬의 delegated worker 모드 절차(§3.1 Mode 결정, §7.2 반환 형식)를 적용하시오. 스킬 path 는 runtime 환경에서 자동 해석.
...
다수 후보는 가능한 경우 순차 단일로 호출한다. 메인은 4분 시간 상한 후 일괄 수신.
...
delegated worker 에 전달하는 prompt 는 다음을 strip:
...
delegated worker 가 반환한 `research-summary.json` 을 per-REQ judgement 에 병합:
...
- 각 delegated worker 호출 = 단일 worker/evaluator 1회. 내부 관점은 순차 처리하며 child-of-child worker 그룹을 만들지 않는다.
- 최악 시나리오 Phase 2.5 단독 비용: `(--research-limit) × (1 + --research-respawn-limit)` 단일 위임 호출. 기본 `5 × (1 + 2) = 15`. 비용 우려 시 limit/respawn-limit 하향 조정.
- **본 스킬 1 run 의 delegated worker 총량 (Phase 1~8 합산, --enable-research 활성 시)**:
  - Phase 1 = 1 (single evaluator 사전조사)
  - Phase 2.0 prescreen = single evaluator chunk review when N≥7
  - Phase 2 per-REQ = main agent or one delegated worker at a time
  - Phase 2.5 = 최대 15
  - Phase 5 evaluator = single evaluator, 3 consecutive clean evaluations
...
- `docs/analysis/kiwi-srs-feasibility-{run-id}/research-enrichment/`: 각 delegated worker 의 research-summary.json 사본
...
10. **Golden rule violation (§0.G1)** — mutation 후 동일 SRS 파일 manual file edit 사용 흔적
...
- 모든 모드: 단일 evaluator 가 개선사항 없음(no actionable improvement)을 **3회 연속** 반환해야 Phase 7 로 진행한다.
- CRITICAL/HIGH/MEDIUM finding 은 개선사항으로 간주하고 §9.3 재spawn 라우팅 후 clean counter 를 0으로 재설정한다.
- LOW finding 은 보고 전용일 때만 clean 판단에서 제외할 수 있다. evaluator 가 actionable 로 표시하면 clean 이 아니다.
- 동일 blocker 가 3회 반복되고 안전한 수정 경로가 없으면 사용자에게 에스컬레이션한다.
...
| 10 §0.G1 황금률 위반 | 작성자 전체 재spawn | mutation 후 manual SRS file edit 발견 시 작성자 교체 |
| **research-derived axis** (5 Blocker substantiation / 2 Evidence existence / 9 Internal coherence 중 finding 의 evidence 가 research-summary 를 인용) AND `--enable-research` 활성 | **Phase 2.5 재spawn** (kiwi-srs-research delegated worker 재호출) | 직전 research-summary 의 dissent/low-confidence 가 finding 근거인 경우 |
...
| 일부 항목 차이 (ok → blocked) | 차이 항목 자동 차단 + 사용자 재승인 게이트 (User clarification gate). 다른 항목은 진행 |
...
- **MCP↔Markdown sync 점검 (전수)** — applied mutation 의 **모든** REQ 에 대해:
  ...
  4. 두 값 불일치 시 **단계적 대응**:
     ...
     | d. 자동 복구 없음 | §0.G1 황금률 (mutation 후 manual SRS file edit 금지) 때문에 자동 정정 불가. 사용자가 speckiwi MCP 동작을 확인하도록 안내 |
...
- 본 run delegated worker 호출 총량 (사전추정/실측): {est}/{actual}
...
CLI 는 MCP 복구 진단에만 참고한다. etc 스킬은 CLI 로 SRS mutation/status/stability 를 수행하지 않는다.

| 작업 | MCP | CLI diagnostic note |
|---|---|---|
| Active target | `get_active_target` | diagnostic read only |
| Target 활성화 | `set_active_target` | MCP required |
| REQ 조회 | `get_requirement` | diagnostic read only |
| REQ 목록 | `list_requirements` | diagnostic read only |
| **Stability 변경** | **`update_stability`** | **MCP required** |
| 검증 | `validate_spec` | diagnostic read only |
| 요약 | `summarize_target` | diagnostic read only |
