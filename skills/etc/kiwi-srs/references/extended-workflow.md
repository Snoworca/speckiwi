...
## Table of Contents
- 9. Phase 4 — SRS write/update (local-LLM max-profile × 1)
- 9.1 작성자 컨텍스트
- 9.2 분류별 MCP 시퀀스
...
- 11.1 심각도
- 11.2 종료 조건
- 11.3 개선 라우팅 (Improvement agent, local-LLM max-profile)
- 11.4 사실 위조 거절 (§0.9)
- 12. Phase 7 — Finalize
- 12.1 검증
...
작성자 local-LLM max-profile 위임 worker에게 전달:
- Phase 1/2/3 산출물
...
§0.G1 결정표 적용. **speckiwi MCP mutation 호출 = Markdown line-patch 1회. 추가 manual file edit 사용 금지.**
...
작성자 local-LLM max-profile 위임 worker 책임:
1. 분류 결과에 따른 §9.2 mutation 시퀀스 호출
2. 시퀀스 종료 후 `validate_spec` 1회 호출 → PASS 확인
3. mutation 도구 외 어떤 방법으로도 `docs/spec/*.srs.md` 파일 수정 금지

**§6.4 게이트 통과 후 §2 Scope Boundaries 변경**: MCP mutation API 가 제공될 때만 수행한다. 도구가 없으면 HALT 하고 scope-boundary 변경 필요성을 보고한다.
...
**검증 서브에이전트 실행(순차, 한 번에 하나씩).**
- 기본, 단일 리서치 문서: 프로세스 A 는 검증 서브에이전트 1개를 돌린다.
- `--max`(etc 기본), 단일 문서 기준: 프로세스 A 는 그 단일 문서에 대해 검증 **서브에이전트를 3회 순차**(3 sequential passes, one at a time)로 돌려 교차 검증한다.
- 다중 리서치 문서, 비-max: 프로세스 A 는 검증 서브에이전트를 **문서별로 순차**(sequential, per document)로 생성한다(문서 1개당 서브에이전트 1개, 한 번에 한 문서씩).
- 다중 리서치 문서, `--max`(etc 기본): 프로세스 A 는 문서 수와 무관하게 검증을 **3회** 순차로(sequential, one at a time) 실행하고 문서 전체를 그 셋에 나눈다 — **문서 수로 곱하지 않는다**. 미검증 노트마다 교차검증을 사는 것은 권위 없는 입력에 비용을 곱하는 것이다.
...
10. **Golden rule violation (§0.G1)** — speckiwi MCP mutation 호출 후 동일 SRS 파일에 manual file edit 사용 흔적 검출. `srs_delta.json.scope_doc_edits` 또는 운영 로그에서 mutation 직후 동일 path manual file edit = **CRITICAL**. 예외: §0.G1 표의 허용 행만.
...
평가·검증은 **단일 검증 서브에이전트**가 수행한다. etc 는 단일 로컬 모델(single local model)로 실행하므로, 별도 모델 선택 없이 현재 로컬 모델을 그대로 사용한다(`--model` 미노출).

| 모드 | 검증 서브에이전트 | 모델 |
|---|---|---|
| Normal | 단일 검증 서브에이전트 × 1 | 단일 로컬 모델 (etc, `--model` 미노출) |
| --max | 단일 검증 서브에이전트 + 독립 2차 검증 패스 | 단일 로컬 모델 (etc, `--model` 미노출) |
...
- LOW finding 은 보고 전용일 때만 clean 판단에서 제외할 수 있다. 단, evaluator 가 LOW를 actionable improvement 로 표시하면 clean 이 아니다.
...
`../../_shared/kiwi/pipeline-event.md` v1.0.0 의 §2 schema 와 §5 emit 패턴을 따라 `./kiwi/pipeline.jsonl` 에 정확히 1줄 append. 멱등성: 동일 `run_id` 의 이벤트가 이미 존재하면 skip.
...
본 표는 §12.2 사용자 보고의 `{next_steps}` 섹션 생성 규칙. 작성자(local-LLM max-profile, Phase 4)는 분류 결과 + 신규/영향 REQ 의 현재 status/stability 를 입력으로 본 표를 순회하며 해당하는 모든 권고를 출력. 표가 도출하지 않은 권고는 추가하지 않음 (pipeline SSOT 와의 정합 보장).
...
CLI 는 MCP 복구 진단에만 참고한다. etc 스킬은 CLI 로 SRS mutation/status/trace/evidence 를 수행하지 않는다.
...
| REQ 추가 | `add_requirement` | MCP required |
| Status 변경 | `update_status` | MCP required |
| Stability 변경 | `update_stability` | MCP required |
| Trace 추가 | `add_trace_link` | MCP required |
| Evidence | `add_verification_evidence` | MCP required |
| AC 체크 | `check_acceptance_criteria` | (MCP 필수) |
| 검증 | `validate_spec` | diagnostic read only |
| 요약 | `summarize_target` | diagnostic read only |
| 목록 | `list_requirements` | diagnostic read only |
