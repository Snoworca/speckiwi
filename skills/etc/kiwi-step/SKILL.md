---
name: kiwi-step
description: "Lightweight step-local SRS authoring skill (opencode / hermes mirror). Claims the target step via claim_step (MCP-only) before authoring, writes requirement drafts only under docs/spec/steps/<name>/ (never into body-scope SRS files), and runs validate_step for step-local verification. Triggers — kiwi step, author step requirement, step-local authoring, claim step, kiwi-step, step SRS draft, /kiwi-step."
---
...
### 2.2 Phase 1 — Claim
Claim the target step via `claim_step` before authoring — complete the claim before writing any requirement. If the claim is rejected by the write-skew gate (STEP_DIRECT_CONFLICT / STEP_OVERLAP / STEP_SUPERSEDE_PROTECTED), report the reason and stop.
