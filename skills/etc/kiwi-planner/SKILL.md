---
name: kiwi-planner
description: "OpenCode/Hermes local-LLM variant for producing implementation plans from active SpecKiwi requirements. Requires speckiwi mcp for requirement reads and trace/evidence updates. Defaults to --max with one evaluator/worker and three clean evaluations. Triggers: kiwi planner, implementation plan, task decomposition."
---
...
| §0.5 | **speckiwi MCP 우선 + 황금률**. mutation 도구 (`add_trace_link` / `add_verification_evidence`) 호출 1회 = 사이드카 `mcp_call_log[]` 1회 = Markdown 자동 line-patch 1회. mutation 후 동일 SRS 파일 manual file edit 금지 (§0.G1) |
| §0.6 | **/snoworca-\* 스킬 호출 절대 금지**. 로직만 차용, 실행은 본 스킬 내부 |
| §0.7 | **사용자 확인 의무**. scope 모호, draft REQ 포함 여부, frozen AC 누락, 외부 모듈 영향 — 모두 User clarification gate 단일 호출 분해 |
| §0.8 | **외부 모듈 수정 금지**. cwd 외부 path 가 Task `files[]` 에 진입 시 즉시 중단 + User clarification gate (§0.G2) |
...
| §0.12 | **`Stability` 진입 가드**. `Stability=deprecated` REQ 는 자동 제외. `Stability=draft` REQ 는 User clarification gate 후에만 진입 (§0.G3 4옵션) |
...
| mutation 호출 후 동일 `docs/spec/*.srs.md` 에 manual file edit 사용 | 차단 + 작성자 재spawn | **CRITICAL** (axis A1) |
| plan.md 자체 갱신 (manual file edit 또는 `UTF-8 file write`) | 허용 (plan.md 는 speckiwi mutation 대상이 아님) | — |
| 사이드카 JSON 수동 manual file edit (mcp_call_log 외 필드) | 허용 단, validator 재실행 필수 | — |

#### §0.G2 — 외부 모듈 영향

| IF | THEN |
|---|---|
| Task `files[]` 에 cwd 외부 path 진입 | 즉시 중단 + User clarification gate 3옵션 |
| Phase 1 code-context analyst 가 외부 경로 보고 | 즉시 중단 + User clarification gate |
| REQ `trace[].reference` 가 외부 path 만 가리킴 | 해당 REQ 를 `unreferenced_reqs` 로 분리 + User clarification gate |
...
| IF | THEN |
|---|---|
| target REQ 중 `Stability=draft` 가 존재 + `--draft-policy=prompt` (기본) | User clarification gate 단일 호출 4옵션 |
...
| User clarification gate 응답 `include-all` | draft 포함, 전체 plan 작성 |
| User clarification gate 응답 `exclude-draft` | draft 제외 후 plan 작성, 사이드카 `excluded_reqs[]` 기록 |
| User clarification gate 응답 `feasibility-first` | 차단 + `$kiwi-srs-feasibility` 선행 권고 후 종료 |
| User clarification gate 응답 `block-all` | 작업 중단 |
...
| IF | THEN |
|---|---|
| coverage 결과 `Stability∈{frozen, stable}` REQ 에 `missing_ac_ids` ≥ 1 | User clarification gate 3옵션 |
...
| IF | THEN |
|---|---|
| 작성자 재호출 3회 누적 | User clarification gate 4옵션 (아래) |
| 평가자 재호출 2회 누적 + 동일 finding 잔존 | User clarification gate 4옵션 + 잔존 finding 사용자 보고 |
| validator C15 (mcp_call_log mismatch) 가 2라운드 연속 잔존 | 즉시 사용자 알림 + plan freeze (아래 정의) |

User clarification gate 4옵션:
...
| IF | THEN |
|---|---|
| 평가자 또는 validator 가 `task.req_ids` 중 target REQ 외 ID 검출 | User clarification gate 3옵션 |
...
|---|---|
| 위 트리거 발동 | User clarification gate 2옵션 (`accept-as-exempt` 비활성) |
...
|---|---|
| 위 트리거 발동 | User clarification gate 3옵션 |
...
- (없음) `get_active_target` 으로 자동 추출. 활성 target 부재 시 User clarification gate.
...
Phase 1  : Pre-investigation (local evaluator × 1 순차 단일: intent / code-context / srs-mapping)
Phase 2  : Plan drafting (local-LLM max-profile 시니어 — Phase>Task 분해, plan.md + 사이드카 동시 작성)
Phase 3  : SRS-satisfaction evaluation (single delegated evaluator, 3 consecutive clean evaluations)
...
4. User clarification gate → "어느 target 의 계획을 수립하시겠습니까?"
...
## 4. Phase 1 — Pre-investigation (local evaluator × 1, 순차 단일, 격리)
...
## 5. Phase 2 — Plan drafting (local-LLM max-profile 시니어)
...
## 6. Phase 3 — SRS-satisfaction evaluation (local-LLM max-profile + local evaluator)

### 6.1 평가자 구성

- default `--max`: single delegated evaluator, 3 consecutive clean evaluations
...
| axis ID | 이름 | 책임 |
|---|---|---|
| A1 | golden_rule | §0.G1 황금률 — mutation 후 manual SRS file edit 흔적 |
...
- 모든 모드: 단일 evaluator 가 개선사항 없음(no actionable improvement)을 3회 연속 반환해야 PASS
- CRITICAL/HIGH/MEDIUM finding 은 개선사항으로 간주하고 Phase 3.5 개선 후 clean counter 를 0으로 재설정
- LOW finding 은 보고 전용일 때만 clean 판단에서 제외 가능. evaluator 가 actionable 로 표시하면 clean 이 아님
...
| `business-decision` | 즉시 User clarification gate. 답변 없으면 plan 진행 차단 |
