...
Handle an urgent bug fix without bypassing governance: write or confirm a
regression test, make the smallest fix, run a strict review, then delegate SRS
catch-up to `kiwi-srs-sync`.
...
| §0.3 | Reviewer and fixer inputs stay isolated; do not pass the fixer rationale to the reviewer. |
...
1. Preflight: verify git, test tooling, SpecKiwi MCP availability, active target,
   and `gh` if a GitHub issue is supplied.
2. Detect input from issue URL, natural-language symptom, or existing working
   tree changes.
