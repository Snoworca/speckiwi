...
| `tdd_review_done` | §4.2 local evaluator×1 결과 통합 후 | { task_id, iter, severity_summary } |
...
| `need_user` | User clarification gate 호출 직후 | { task_id, reason, options[] } |
...
2. 통합 테스트 작성 (local-LLM max-profile 시니어, Phase 간 연계 / end-to-end / 경계)
...
3. 계획-코드 매핑 결과
4. local evaluator×1 TDD 검증 통계 (round별)
...
$kiwi-coder PLAN_PATH=... local-LLM max profile
