# `--mini` / `--loops` loop-counter option — SSOT v1.0 (etc Kiwi skills)

Shared definition of the options that bound the **verify → improve → re-verify loop round cap** for
OpenCode/Hermes/local-LLM `kiwi-*` skills. Interpret together with `local-llm-profile.md`: multi-worker
fanout is disabled, so verifier/evaluator rounds run **sequentially** (one delegated worker at a time).
The round-cap semantics below are **identical** to the claude/codex variants — a round cap bounds the
number of sequential rounds.

> **Note**: this `--mini` is a **round-cap preset**. It is unrelated to the model-swap `--mini` removed
> by FR-FLOW-022. Model selection remains `--model <name>` only, and the former model-swap shared SSOT
> deleted by FR-FLOW-022 is NOT revived.
...
- A **round** = one verify → improve cycle (evaluator runs → findings fixed → re-verify). Under the etc
  single-worker profile these rounds are executed sequentially.
...
(auto-option §11.1; pipeline explicit `next_hint`; feasibility explicit `--research-respawn-limit`;
FR-FLOW-022 `--model`) and the general CLI convention (gcc `-O2` + `-fno-*`, rsync `-a` + `--no-perms`,
ESLint extends override). Unanimous 3-subagent decision —
`docs/analysis/kiwi-loop-option-2026-07-12.speckiwi.v2301/priority-research.md`.
...
- `--max` controls **verification strength** (sequential evaluator passes / gate strictness / committee &
  divergence escalation) and is **orthogonal** to the round cap.
