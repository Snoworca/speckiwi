...
description: "OpenCode/Hermes local-LLM variant of the multi-wave orchestrator. Decomposes a large task (epic, multi-step roadmap, long research doc) into ordered waves, registers a scoped wave-{n} target per wave via $kiwi-srs, then runs a per-wave $kiwi-pipeline in order. Tracks progress in ./kiwi/waves.jsonl and is resumable from the first incomplete wave. Requires speckiwi mcp when SRS state is touched. Defaults to --max and never fans out to multiple workers. Triggers: kiwi wave master, multi wave orchestration, decompose large task into waves. Options: --auto (run all waves to the end, per-wave safety gates still apply), --max (propagate to every wave's sub-skills)."
---
...
| 키 | 규칙 |
|---|---|
| §0.1 | **이벤트 SSOT**: `../_shared/kiwi/waves-event.md` v1.3.0 가 `./kiwi/waves.jsonl` 의 schema·파일위치·mark-complete 규칙 SSOT. 본 문서는 wave 분해·오케스트레이션 로직만 담당. |
...
| "max 모드", "고강도", "최대로" | `--max` (모든 wave 의 하위 스킬로 전파) | on (etc 기본) |
...
| "루프 N회", "N라운드" | `--loops N` (per-wave kiwi-srs/kiwi-pipeline 로 전파 §7.3) | off (스킬 기본 상한) |
...
Phase 3.5 : 웨이브 종료 상호검증 (§5.5) — 검증 축 2 개 교차반박 루프(순차), 통과해야 complete
...
### 3.1 설계 기준선(design baseline) 물질화
...
이 매핑을 `docs/analysis/kiwi-wave-master-{run_id}/design-baseline.json` 로 영속하고, 그 경로를 각 wave 의 **첫 `in_progress`** 이벤트의 `design_baseline` 필드에 실어 `waves.jsonl` 에 기록한다.
...
wave 별 설계 **본문 발췌**를 `docs/analysis/kiwi-wave-master-{run_id}/design-baseline/wave-{n}.md` 로 함께 물질화하고, 그 경로를 `design_baseline` 의 `excerpt_path` 에 기록한다 — 증거 번들과 저작 입력이 같은 아티팩트를 가리키게 한다.
...
루프 자체는 §5.5 를 그대로 재사용한다: **정확히 2개의 검증 축**을 stance 로 분리해 순차 수행하고, 교차반박은 `add-only` 이며, clean 라운드는 **그 라운드에서 수정이 적용되지 않았을 것**을 요구한다. 라운드 상한과 `--mini` / `--loops N` 도 §5.5.4 를 따르고, finding 의 수정 라우팅은 §5.5.5 를, wave 로 귀속되는 finding 의 이월은 §5.5.7 을 그대로 쓴다 — 재사용 목록에서 이 둘이 빠지면 최종 패스의 finding 은 전부 HALT 로만 끝난다.
...
`--max` 활성 시 **모든 wave** 의 `$kiwi-pipeline` 과 그 **하위 스킬(sub-skill)** — 각 wave 사이클이 spawn 하는 `kiwi-srs` · `kiwi-srs-feasibility` · `kiwi-planner` · `kiwi-pm` · `kiwi-review-fix-loop` — 에 `--max` 를 그대로 **전파(propagate)** 한다. 하위 스킬의 `--max` 의미는 각자의 SSOT 를 따른다. (etc 프로파일은 `--max` 를 기본으로 둔다.)
