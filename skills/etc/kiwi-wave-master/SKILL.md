...
| 키 | 규칙 |
|---|---|
| §0.1 | **이벤트 SSOT**: `../_shared/kiwi/waves-event.md` v1.0.0 가 `./kiwi/waves.jsonl` 의 schema·파일위치·mark-complete 규칙 SSOT. 본 문서는 wave 분해·오케스트레이션 로직만 담당. |
...
| "max 모드", "고강도", "최대로" | `--max` (모든 wave 의 하위 스킬로 전파) | on (etc 기본) || "N 번째 wave 부터", "이어서" | `--resume` (첫 미완료 wave 부터 재개) | 자동 감지 |
