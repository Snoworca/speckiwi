...
## Table of Contents
- 6. Phase R2 — Synthesis (local-LLM max-profile × 1)
- 6.1 입력 (4종 raw)
- 6.2 책임
- 6.3 금지 (§0.G4)
...
- 영속화
- 다음 단계
- 7.2 delegated worker 모드
- 8. MCP diagnostics
- 9. 수렴 기준
- 10. 검증 축 (synthesis step 자체 검증 + 호출자 측 점검)
- 11. 주의사항
- 12. 파이프라인 위치
...
1. **합의 추출**: ≥2 perspective 가 동일하거나 호환되는 claim 식별 → `consensus_findings`
...
- `from`: code_perspective / external_perspective / risk_perspective / triage
- `quote`: raw 입력의 *원문 그대로 인용* (paraphrase 금지)
- `raw_path`: raw 파일 내부 JSON path (예: `research-raw/code-research.json#findings[2].claim`)

본 스킬은 synthesis step 출력 후 자동 검증 단계를 수행:
1. 각 source_quote 의 `quote` 가 `raw_path` 위치의 텍스트와 **literal string match** 되는지 확인 (결정적 검증, 자기검증 편향 없음)
2. 매칭 실패 발견 시 `§10 axis 8 CRITICAL` finding 발생 + synthesis step 재spawn, clean counter = 0
3. 매칭 성공 후 단일 evaluator 가 "claim 이 quote 의 의미를 보존하는가? 의역/확장/축소 여부" 를 판정한다.
4. evaluator 가 의역 의심, 근거 부족, 또는 hallucination risk 를 보고하면 개선사항으로 처리하고 synthesis step 재실행 후 clean counter = 0
5. evaluator 가 개선사항 없음(no actionable improvement)을 **3회 연속** 반환해야 synthesis verification 을 통과한다.
6. 불확실하지만 차단 근거가 부족한 항목은 `review_warnings[]` 에 기록하고 사용자 샘플 검토를 권장한다.
...
{
  "research_question": "...",
  "mode": "standalone|delegated worker",
  "consensus_findings": [
    {
      "claim": "...",
      "supported_by": ["code_perspective", "external_perspective"],
      "evidence": ["src/api.ts:L45-67", "context7:react/hooks#XYZ"],
      "source_quotes": [
        { "from": "code_perspective", "quote": "raw 인용 텍스트 (원문 변형 금지)", "raw_path": "research-raw/code-research.json#findings[2]" }
      ],
  ...
  "dissent_findings": [
    {
      "claim": "...",
      "supporter": "risk_perspective",
      "conflicting_views": [
        { "by": "code_perspective", "view": "..." }
      ],
      "resolution_suggested": "user_decision_required | additional_research_needed | accept_dissent_as_minor"
    }
  ],
  "suggested_mitigations": [
    { "for_finding": "claim_id_or_text", "mitigation": "...", "source": "risk_perspective" }
  ],
  "open_questions": [],
  "paraphrase_detector_disagreements": [
    { "claim_id_or_text": "...", "local evaluator_verdict": "literal", "synthesis step_verdict": "paraphrased", "claim_review_recommended": true }
  ],
...
`input_bias_warnings`:
- delegated worker 모드에서 §0.G5 strip 이 발생한 경우만 비어있지 않음
...
- 대상: REQ {REQ_ID} (또는 질문)
- single-worker 토폴로지: one delegated worker/evaluator; code/external/risk perspectives processed sequentially
...
`docs/analysis/kiwi-srs-research-{run-id}/` 디렉토리는 동일 작성 (감사 추적). `mutation-log.json` 은 `{ mode: "delegated worker", mutations: [] }` 로 비어있음.
...
- Triage 가 3 관점 패키지 모두 생성 (누락 시 재시도)
- 코드/외부/위험 관점 결과가 모두 기록됨 (시간 초과 시 부분 결과 + 경고)
- synthesis step 가 모든 raw 입력을 참조 (참조 누락 시 재시도)
- §0.G4 무결성 검증 통과 (입력에 없는 claim 0건)
- standalone 모드: `append_section_note` 호출 결과 ok
- delegated worker 모드: mutation 호출 0건 확인
...
| 5. perspective isolation | A/B/C 가 서로 출력 공유 흔적? (cross-reference 발견 시 위반) | CRITICAL |
| 6. Consensus correctness | synthesis step 의 consensus 가 실제로 ≥2 supporter? | HIGH |
| 7. Dissent preservation | synthesis step 가 이견을 합의로 위장? (§0.11) | HIGH |
| 8. Hallucination | synthesis step 출력에 raw 4종에 없는 claim? (§0.G4) | CRITICAL |
| 9. Golden rule (standalone) | mutation 후 동일 SRS 파일 manual file edit? (§0.G1) | CRITICAL |
| 10. Mode boundary (delegated worker) | delegated worker 모드에서 mutation 시도? (§0.G3) | CRITICAL |

평가자 토폴로지는 본 스킬에선 별도 Phase 없음. 검증 책임은:
- standalone: 호출 후 사용자가 보고서 검토 + 필요 시 `kiwi-srs-reviewer` 호출 (계획)
- delegated worker: 호출자(예: kiwi-srs-feasibility) 가 자체 검증 축에 본 표 일부 포함

---

## 11. 주의사항

- **single-worker 토폴로지 고정** — 사용자가 옵션으로 줄이거나 늘릴 수 없음 (§0.5)
- standalone 모드 mutation = `append_section_note` 만. status/stability 변경 금지 (§0.13)
- delegated worker 모드는 read-only — JSON 반환만, 파일/MCP 부수효과 0건 (§0.7)
- 관점별 입력 격리 — 서로 중간 추론 공유 금지 (§0.10)
- 종합 단계는 raw 입력에 없는 claim 추가 금지 (§0.4, §0.G4)
- 이견 보존 의무 — 합의로 위장 금지 (§0.11)
- 외부 모듈 영향 발견 시 해당 연구 단위 중단 (§0.G2)
- `/snoworca-*` 호출 절대 금지 (§0.8)
- 비용 관리: 1 연구 호출 = single worker/evaluator 1회. 호출자가 budget 관리 책임
- delegated worker 모드 호출자는 자기 결론·판정·정당화를 본 스킬 prompt 에 주입 금지 (§0.15, §0.G5). 위반 시 본 스킬이 strip 후 진행하지만 `input_bias_warnings` 로 호출자에 회신
...
2. **delegated worker**: `kiwi-srs-feasibility` 가 feasibility ∈ {medium, low} 인 REQ 의 블로커 모호성 해소를 위해 Phase 2.5 에서 호출 (kiwi-srs-feasibility §1.2 `--enable-research`)
...
- **delegated worker 모드 (호출자에서 spawn)**: 본 스킬은 emit 하지 않는다 — 호출자(`kiwi-srs-feasibility`) 의 이벤트가 SSOT. 본 스킬 결과는 호출자의 `notes` 에 인용.
