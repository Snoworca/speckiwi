...
- `GITHUB_ISSUE` — GitHub 이슈 번호(issue number, 예: `#123`) 또는 이슈 본문/컨텍스트. `kiwi-pipeline` 의 이슈 진입 흐름(FR-FLOW-028)이 이슈 해결(resolution) 방향 + 구현 접근(implementation-approach) 연구를 위해 본 입력으로 전달한다. 이슈 본문·연결 정보를 `RESEARCH_QUESTION` 으로 정규화하여 single-worker 순차 연구 토폴로지(§4 Triage → §5 Sequential Research)에 투입한다. 이슈 본문은 `gh issue view {N} --json title,body,comments` 로 읽는다 — 저장소의 다른 스킬이 같은 일을 하는 자리와 같은 필드 목록이다.
