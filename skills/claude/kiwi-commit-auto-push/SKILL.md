...
### 11.6 인계 프로토콜 — child 모드 전용 (`references/conditional-sections.md`)

`/kiwi-coder` 또는 `/kiwi-pm` 가 본 스킬을 spawn 한 **child 모드**에서만 적용된다. 그 경우에만 `references/conditional-sections.md` 의 §11.6 (동작 변경 · 3상태 반환 JSON SSOT · 게이트 매핑)을 읽는다. 단독 호출은 이 절을 읽지 않는다.
