---
name: kiwi-coder
description: "kiwi-planner 산출물(plan_contract=1.2.0 + sidecar TDD)을 입력 SSOT 로 받아 Task 단위로 TDD 선행 → Sonnet×2 병렬 TDD 검증 → Opus 시니어 구현 → 정형 검사 → 까칠 리뷰 → 개선 루프 → 테스트 실행 → 회귀 검증 → speckiwi MCP mutation → .kiwi/ 상태 갱신을 자동화하는 코딩 스킬 v0.1. 재개 가능. 트리거: 계획대로 구현, kiwi 코딩, tdd 코딩, plan 구현, kiwi planner 산출물 구현. 검증(정형 검사·까칠 리뷰) 서브에이전트는 현재 세션 모델을 상속하며 `--model <name>` 로 그 모델을 override 한다(TDD 검증 Sonnet×2 불변)."
---
...
| 키 | 규칙 |
|---|---|
| §0.1 | **TDD 강제**. 모든 코딩 Task 는 (1) 테스트 작성 → (2) Sonnet×2 병렬 검증 → (3) red 실패 확인 → (4) 구현 → (5) green 확인 순서. 우회 금지 |
...
| §0.16 | **검증 서브에이전트 모델 정책 SSOT**. 까칠 리뷰 등 검증 서브에이전트는 기본적으로 **현재 세션 모델(current session model)**을 상속한다. `--model <name>` (또는 사용자가 지명한 모델) 로 검증 서브에이전트의 모델을 override 한다 (시니어 코더는 영향 없음). **TDD 검증 Sonnet×2 (Phase 1.2) 는 모든 모드 공통 유지 (§0.1)** — 모델 불변. 심각도 게이트·회귀 테스트 의무는 불변 |
...
| Phase 1 검증(Sonnet×2 + 스크립트 판정 2종)에서 1개라도 CRITICAL finding 잔존 | Phase 2 (구현) 진입 차단, Phase 1.3 개선 루프 |
...
| `improvement-loop-divergence-4opt` | §0.G4 4옵션 게이트 발동 (시니어/Sonnet×2/까칠 누적 + 회귀 2회 연속 fail) | §0.G4 / §7.4 |
...
| 모드 | 시니어 코더 | TDD 검증 (Sonnet) | 정형 검사 (스크립트) | 까칠 리뷰어 (현재 세션 모델) | 비용 배수 |
|---|---|---|---|---|---|
| Normal (기본) | Opus × 1 | × 2 (병렬) | × 1 | × 1 | 2.0~2.5× (snoworca-coder Normal 대비) |
| `--max` | Opus × 3 | × 2 | × 1 | × 2 | 12~15× |
| `--reviewer-off` | Opus × 1 | × 2 | × 1 | × 0 | 1.6× |

`--model <name>` 지정 시 까칠 리뷰 검증 서브에이전트의 모델을 override (정형 검사는 스크립트라 대상 아님) (기본은 현재 세션 모델; 시니어 코더·TDD 검증은 영향 없음).

TDD 검증 (Sonnet×2) 는 **모든 모드 공통**. TDD 강제 원칙 (§0.1) 의 핵심 검증 채널이므로 모드 변경 영향 없음.
...
- **분석 로그**: `docs/analysis/kiwi-coder-{run-id}/`
  - `tdd_review_iter{N}.json` (Sonnet×2 결과 통합)
...
### 4.2 Sonnet×2 병렬 TDD 검증 (2축)

**모든 모드 공통**. Sonnet 2 인스턴스를 단일 메시지의 2개 Agent 호출로 병렬 spawn. 각 검증자는 시니어의 rationale 미수신 (§0.5). 나머지 2축(req-mapping·red-verification)은 아래 표대로 **스크립트가 판정한다**.
...
|---|---|---|---|
| **S1** intent-alignment | Sonnet | 계획 의도/AC ↔ test 의미 일치 — test 가 정말 해당 AC 를 검증하는가 | `{ severity, axis: "intent-alignment", evidence: {file, line}, suggestion }` |
...
| `tdd_review_done` | §4.2 Sonnet×2 결과 통합 후 | { task_id, iter, severity_summary } |
