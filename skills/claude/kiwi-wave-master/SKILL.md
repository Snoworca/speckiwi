...
| 키 | 규칙 |
|---|---|
| §0.1 | **이벤트 SSOT**: `~/.claude/skills/_shared/kiwi/waves-event.md` v1.3.0 가 `./kiwi/waves.jsonl` 의 schema·파일위치·mark-complete 규칙 SSOT. 본 문서는 wave 분해·오케스트레이션 로직만 담당. |
...
Phase 3.5 : 웨이브 종료 상호검증 (§5.5) — 검증자 2 기 교차반박 루프, 통과해야 complete
...
라운드 상한(`~/.claude/skills/_shared/kiwi/loop-option.md` v1.0): 기본 **5**, `--max` **8**, `--mini` 3. `--mini` 와 `--loops N` 동시 지정 시 **`--loops` 우선**(비치명 **WARN**).
