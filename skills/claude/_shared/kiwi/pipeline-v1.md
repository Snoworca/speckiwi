...
This shared reference is the contract for the `kiwi-*` pipeline. Load it when a
skill needs pipeline ownership, next-step routing, end-to-end cycle
orchestration, or final event emission rules.
...
Use `pipeline-event.md` for the event schema and emit rules. The single-step
routing summary is:
...
- Ask the user directly for business decisions; use `AskUserQuestion` for gated
  choices that are not auto-decidable.
