...
git -C <lane-root> checkout <base_sha>        # 의무. 생략 금지
```

**명시 `checkout` 은 선택이 아니다.** 실측: 런타임이 만든 워크트리의 기본 HEAD 는 `origin/<기본 브랜치>` 였고, 작업 중인 브랜치보다 **114 커밋 뒤**였다. 그 상태로 고친 diff 는 **깨끗하게 병합된다** — 몇 달 전 코드를 고쳤다는 사실이 어디에도 드러나지 않는다.
