...
```bash
grep -q "\"run_id\":\"<rid>\"" "$PIPE_DIR/pipeline.jsonl" 2>/dev/null && echo "[pipeline] event already emitted for run_id=<rid>, skipping" && exit 0
```
...
```
- **Pipeline emit (의무)**: `~/.claude/skills/_shared/kiwi/pipeline-event.md` v1.0.0 를 따라 종료 이벤트 1줄 append. 멱등성 보장 (run_id 기준).
```
...
- 이벤트를 *읽고* 다음 단계를 추천하는 로직은 `~/.claude/skills/kiwi-pipeline/SKILL.md` 에 별도.
