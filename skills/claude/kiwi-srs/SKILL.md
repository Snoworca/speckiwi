---
name: kiwi-srs
description: "신규 요구사항을 받아 기존 코드 + speckiwi MCP SRS 데이터베이스와 교차 분석하여 conflict/update/new-feature/new-scope 4방향 분류 → 구현 가능성 판정 → speckiwi MCP를 SSOT로 SRS 증분 작성·갱신. 3 Sonnet 사전조사 병렬 + Opus 시니어 작성자 + Opus×1+Sonnet×1 평가자(Max는 Opus×2+Sonnet×1) + 심각도 게이트(Normal: CRITICAL=0+HIGH=0 / Max: 2연속 MEDIUM-zero). 트리거 — SRS 업데이트, 요구사항 추가, kiwi srs 써줘, 기존 SRS에 반영해줘, 새 기능 요구사항 SRS, SRS 충돌 감지, 충돌 SRS 확인, 증분 SRS authoring, 신규 기능 SRS, 요구사항 명세 갱신, speckiwi 요구사항 등록, srs conflict 분석, kiwi srs 작성, incremental srs, add requirement to srs, update existing SRS, new feature spec, kiwi requirement authoring. **기본 QnA 활성** (Agent Dropout, Normal 3/Max 7 라운드, Phase 1 모호성 0건 시 skip). **--auto 로 질문 없이 진행** (현재의 무옵션 동작과 등가). `--qna` 는 v0.11 까지 deprecated alias. --max로 평가자 승격(Opus×2+Sonnet×1, 2연속 MEDIUM-zero 종료). --mini로 비용 절감(모든 Opus→Sonnet override, `_shared/kiwi/mini-option.md` v1.0 — 토폴로지·게이트 불변). 종료 시 ./kiwi/pipeline.jsonl 에 이벤트 1줄 append (kiwi-pipeline 메타 스킬용)."
---
...
```json
{
  "evaluator": "opus-A|sonnet-B|opus-C",
  "iter": N,
  "findings": [
    {
      "id": "{axis}-{iter}-{seq}",
