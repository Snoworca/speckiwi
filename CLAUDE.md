...
- **Research documents (`docs/research/**`)**: LLM review is never run over them, not merely capped. They are unverified working notes and each one says so at its top; every claim that matters is re-checked against code when it becomes a requirement, and that check is strictly stronger than reading the prose. Research 18 and 19 are the evidence — each shipped wrong claims that survived review and fell within minutes to an implementer running the code. Mechanical checks (the notice, links) still apply.
