 ...
 * absent or invalid state.md (missing file, missing/malformed Mode, or a Mode
 * value outside {sdd, vibe, wait}) yields Mode=wait via parseStepState rather
 * than throwing. setWorkMode persists Mode and, for vibe, the Active Task name
 * back into that same metadata block.
 */
 ...
 * or invalid. parseStepState already encodes the fail-open contract, so an absent
 * stateFile is treated identically to an unparseable one.
 */
export async function getWorkMode(root: ProjectRoot): Promise<WorkMode> {
  ...
  return parsed.activeTask !== undefined && parsed.mode === "vibe"
 ...
 * vibe (it is meaningful only there per parseStepState).
 */
export async function setWorkMode(
  ...
  if (input.mode === "vibe" && input.activeTask !== undefined) {
    ...
    // FND-001: switching to a non-vibe mode (sdd/wait), or vibe without an Active Task, must drop any
    // existing Active Task line. Otherwise state.md becomes self-inconsistent (e.g. `Mode: wait` left
    // beside a stale `Active Task: T-OLD`) and the vibe-gate would later evaluate the stale task.
    const activeLine = findMetadataField(stateFile, limit, /^\s*Active Task:\s*/);
    if (activeLine >= 1) {
