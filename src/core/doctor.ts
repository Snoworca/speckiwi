...
import type { Diagnostic, DoctorCheck, DoctorResult } from "./dto.js";
import type { DoctorInput } from "./inputs.js";
...
async function packageVersionCheck(rootPath: string): Promise<DoctorCheck> {
...
async function requiredFilesCheck(speckiwiPath: string): Promise<DoctorCheck> {
...
async function cacheCheck(root: Parameters<typeof buildCacheInputs>[0], workspace: Parameters<typeof buildCacheInputs>[1]): Promise<DoctorCheck> {
...
async function mcpBinaryCheck(): Promise<DoctorCheck> {
...
function stdioPolicyCheck(): DoctorCheck {
  return {
    id: "stdio_policy",
...
function diagnostic(code: string, message: string, severity: Diagnostic["severity"] = "error", path?: string): Diagnostic {
  const output: Diagnostic = { code, message, severity };
  if (path !== undefined) {
    output.path = path;
  }
  return output;
