...
export function bindRequirementPayloadShards(index: EntityIndexV1, shardRefs: RequirementPayloadShardRef[]): EntityIndexV1 {
  const shardByRequirementId = new Map<string, RequirementPayloadShardRef>();
...
export function buildRequirementPayloadShardRefs(shards: RequirementPayloadShardV1[]): RequirementPayloadShardRef[] {
  return shards
    .map((shard) => ({
...
export function buildRequirementPayloadShards(
  registry: RequirementRegistry,
  documentHashes: Map<string, string>
): RequirementPayloadShardV1[] {
  const requirementsByPath = new Map<string, Array<(typeof registry.requirements)[number]>>();
  ...
  const shards: RequirementPayloadShardV1[] = [];
  ...
  for (let offset = 0; offset < documents.length; offset += MAX_DOCUMENTS_PER_REQUIREMENT_SHARD) {
    ...
    const requirements = chunk.flatMap((document) =>
      (requirementsByPath.get(document.path) ?? []).map((requirement, ordinal) => ({
        id: requirement.id,
        ordinal,
...
function documentHashForSingleShard(
  document: RequirementRegistry["documents"][number] | undefined,
  requirements: RequirementPayloadShardV1["requirements"],
  documentHashes: Map<string, string>
): string {
  ...
  return (
    documentHashes.get(document.path) ??
    sha256(
      JSON.stringify({
...
export function deserializeEntityIndex(value: unknown): RuntimeEntityIndex | undefined {
  ...
  return {
    ...
    requirementShardsById: new Map(
      index.requirementPayloadShards.flatMap((shard) => shard.requirementIds.map((id) => [id, shard] as const))
    )
  };
}

export function deserializeRequirementPayloadShard(value: unknown): RequirementPayloadShardV1 | undefined {
  const shard = objectValue(value);
  if (
    shard?.format !== REQUIREMENT_PAYLOAD_SHARD_FORMAT ||
...
export function requirementPayloadShardStorePath(documentHash: string): string {
  return `${REQUIREMENT_SHARD_DIRECTORY}/${documentHash}.json`;
}

function entityIndexValue(value: unknown): EntityIndexV1 | undefined {
  const index = objectValue(value);
  if (
    index?.format !== ENTITY_INDEX_FORMAT ||
  ...
  if (
    documents.length !== index.documents.length ||
...
function documentSummaryValue(value: unknown): EntityDocumentSummary | undefined {
  ...
  return {
    id: item.id,
    type: item.type as DocumentSummary["type"],
    path: item.path,
...
function requirementSummaryValue(value: unknown): EntityRequirementSummary | undefined {
  const item = objectValue(value);
  if (
    typeof item?.id !== "string" ||
    typeof item.type !== "string" ||
...
function scopeValue(value: unknown): RegisteredScope | undefined {
  const item = objectValue(value);
  if (
    typeof item?.id !== "string" ||
    typeof item.index !== "number" ||
...
function shardRefValue(value: unknown): RequirementPayloadShardRef | undefined {
  const item = objectValue(value);
...
function isProjectSummary(value: unknown): value is EntityIndexV1["project"] {
  const item = objectValue(value);
  return (
    typeof item?.id === "string" &&
...
function tupleArray(value: unknown): value is Array<[string, number]> {
  return (
    Array.isArray(value) &&
    value.every(
      (item) =>
