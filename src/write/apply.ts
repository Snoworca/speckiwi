...
import type { Diagnostic, DiagnosticBag, JsonObject, ApplyResult } from "../core/dto.js";
import type { ApplyChangeInput, CacheMode } from "../core/inputs.js";
...
type ApplyFailureCode =
  | "APPLY_REJECTED_CONFIRM_REQUIRED"
  | "APPLY_REJECTED_INVALID_INPUT"
  | "APPLY_REJECTED_ALLOW_APPLY_FALSE"
  | "APPLY_REJECTED_PROPOSAL_NOT_FOUND"
  | "APPLY_REJECTED_STALE_PROPOSAL"
  | "APPLY_REJECTED_VALIDATION_ERROR"
  | "APPLY_REJECTED_LOCK_CONFLICT"
  | "APPLY_REJECTED_ATOMIC_WRITE_FAILED"
  | "APPLY_REJECTED_TARGET_INVALID";
...
async function findProposalById(root: WorkspaceRoot, proposalId: string): Promise<StorePath> {
...
async function staleDiagnostics(root: WorkspaceRoot, proposal: ProposalDocument): Promise<DiagnosticBag | undefined> {
...
async function validateUpdatedWorkspace(root: WorkspaceRoot, targetStorePath: StorePath, raw: string): Promise<DiagnosticBag> {
...
async function allowApply(root: WorkspaceRoot): Promise<boolean> {
  try {
...
async function writeBackup(root: WorkspaceRoot, targetStorePath: StorePath): Promise<string> {
...
async function markCacheStale(root: WorkspaceRoot, modifiedFiles: string[]): Promise<void> {
  ...
  await atomicWriteText(
    ...
    `${JSON.stringify({ stale: true, modifiedFiles, updatedAt: new Date().toISOString() }, null, 2)}\n`
  );
}

function hasExactlyOneChangeSource(input: ApplyChangeInput): boolean {
  return [
